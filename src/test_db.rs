    ...
    F: FnMut(u64) -> Result<(KeyIndex, String)>,
{
    ...
    info!("Total entry count: {}, testing {} entries from 0 to {}", entry_count, entries_to_test, entry_count);
    ...
    // 创建进度报告
    let mut progress_report = ProgressState::new("dump_db", entries_to_test as u64, 5, Some(print_progress));
...
pub fn test_mdx(file_path: &PathBuf, test_count: Option<usize>, random: bool) -> Result<()> {
    let mdx_url = Url::from_file_path(file_path).map_err(|_| mdx::ZdbError::invalid_path(format!("{}", file_path.display())))?;
...
pub fn run_test_db(path: &str, mdx_mode: bool, test_count: Option<usize>, random: bool) -> mdx::Result<()> {
    ...
    if target.is_dir() {
        ...
        let dir_url = Url::from_directory_path(&target).map_err(|_| mdx::ZdbError::invalid_path(format!("{}", &target.display())))?;
