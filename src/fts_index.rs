...
pub fn search_mdx_fulltext(file_path: &PathBuf, query: &str, max_results: usize) -> Result<()> {
    ...
    if !mdx_reader.is_fts_available() {
        ...
        println!("Please create an index first using: mdx_util create-index {}", file_path.display());
        return Ok(());
    }
        ...
        if let Ok(key_index) = mdx_reader.get_index(*entry_no as mdx::storage::key_block::EntryNo) {
            match mdx_reader.get_html(&key_index) {
                Ok(html_content) => {
                    ...
                    println!("Content Preview: {}", utils::take_chars(&text_content, 1024));
                }
                Err(_) => {
...
pub fn run_create_index(mdx_file_path: &str) -> mdx::Result<()> {
    ...
    let target = mdx::utils::io_utils::fix_windows_path_buf(PathBuf::from(shellexpand::tilde(mdx_file_path).to_string()));
    ...
    if target.extension().and_then(|s| s.to_str()) != Some("mdx") {
        ...
        return Err(mdx::ZdbError::invalid_path(format!("Invalid file extension: {}", target.display())));
    }
